...
module forward_unit (
    ...
    input wire       i_mem_mem_to_reg,   // <-- NEW
    ...
    // =========================
    assign o_forward_a =
        // MEM → EX (only if NOT a load)
        (i_mem_reg_write && !i_mem_mem_to_reg &&
         (i_mem_rd != 5'd0) &&
         (i_mem_rd == i_ex_rs1)) ? 2'b10 :

        // WB → EX (covers loads + ALU)
        (i_wb_reg_write &&
         (i_wb_rd != 5'd0) &&
         (i_wb_rd == i_ex_rs1)) ? 2'b01 :
    ...
    assign o_forward_b =
        ...
        (i_mem_reg_write && !i_mem_mem_to_reg &&
         (i_mem_rd != 5'd0) &&
         (i_mem_rd == i_ex_rs2)) ? 2'b10 :

        // WB → EX (covers loads + ALU)
        (i_wb_reg_write &&
         (i_wb_rd != 5'd0) &&
         (i_wb_rd == i_ex_rs2)) ? 2'b01 :
